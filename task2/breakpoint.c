...
int main(int argc, char** argv)
{
    ...
    if (argc < 3) {
        fprintf(stderr, "Expected: child_name Instr_addr\n");
        return -1;
    }

		unsigned long ins_addr = 0;
		sscanf(argv[2], "%x", &ins_addr);
    ...
    if (child_pid == 0)
        ...
        run_target(argv[1]);
				...
				// 父进程
        run_debugger(child_pid, ins_addr);
    else {
        perror("fork");
        return -1;
    }
